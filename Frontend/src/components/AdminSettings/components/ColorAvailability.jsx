...
const ColorAvailability = () => {
  ...
  const onSuccess = index => {
    ...
    const element = document.getElementsByClassName('color-availability-mark')[
      allColors[index].toUpperCase()
    ];
  ...
  useEffect(() => {
    const availabilityMark = document.getElementsByClassName(
      'color-availability-mark',
    );
    if (!(availabilityMark && availabilityMark.length)) {
      if (
        ...
        allColorsAvailable &&
        allColors.length &&
        allColorsAvailable.length
      ) {
        const colorCircles = document.getElementsByClassName(
          'circle-picker ',
        )[0];
